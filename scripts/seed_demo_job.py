...
- Mix of auto-verified and needs-review blocks (amber verdict)
...
import fx_enrich
import aggregate
import risk_scoring   # review: validate z-score against SAS baseline
...
# SAS: sas/05_risk_scoring_iml.sas:3 (PROC IML — z-score)
# Review: validate sample-std (ddof=1) and zero-guard against SAS baseline.
import numpy as np
import pandas as pd
...
BLOCK_PLANS = [
    ...
    # ── 05_risk_scoring_iml.sas ────────────────────────────────────────
    {
        ...
        "risk": "medium",
        "rationale": (
            "PROC IML matrix operation translated to NumPy."
            " Validate sample-std (ddof=1) and zero-guard behaviour against"
            " the SAS baseline before accepting."
        ),
...
MIGRATION_PLAN = {
    "summary": (
        ...
        "10 of 15 steps are fully auto-translated. Five steps need targeted review: "
        "the PROC IML z-score computation, the XLSX import, two assert_rowcount macro "
        "expansions, and the PROC MEANS hierarchical output."
    ),
    "overall_risk": "medium",
    "risk_explanation": (
        "Medium overall risk. The majority of the pipeline consists of straightforward "
        "SQL joins and DATA step filters. The PROC IML z-score computation has been "
        "translated to NumPy and flagged for validation. The remaining review items "
        "are targeted and not blocking."
    ),
...
LINEAGE = {
    ...
    "file_nodes": [
        ...
        },
        {
            ...
            "status": "ERROR_PRONE",
            "status_reason": (
                "PROC IML z-score block translated to NumPy — validate output"
                " distribution against SAS baseline before accepting"
            ),
        },
        {
    ...
    "pipeline_steps": [
        ...
        },
        {
            ...
            "description": (
                ...
                " identical. PROC IML translated to NumPy — review output distribution"
                " against SAS baseline before accepting."
            ),
