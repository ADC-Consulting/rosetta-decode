...
def test_explain_make_agent_tensorzero_path() -> None:
    ...
    with (
        patch("src.worker.engine.chatbot.explain_agent.worker_settings", mock_settings),
        patch(
            "src.worker.engine.chatbot.explain_agent.OpenAIProvider",
            return_value=mock_tz_provider,
        ),
        patch(
            "src.worker.engine.chatbot.explain_agent.OpenAIChatModel",
            return_value=mock_model,
        ) as mock_oai_model,
        patch("src.worker.engine.chatbot.explain_agent.Agent", return_value=mock_agent),
    ):
        from src.worker.engine.chatbot.explain_agent import _make_agent

        result = _make_agent("system_prompt_text")
...
def test_explain_make_agent_azure_path() -> None:
    ...
    with (
        patch("src.worker.engine.chatbot.explain_agent.worker_settings", mock_settings),
        patch(
            "src.worker.engine.chatbot.explain_agent.AzureProvider",
            return_value=mock_azure_provider,
        ) as mock_az,
        patch("src.worker.engine.chatbot.explain_agent.OpenAIChatModel", return_value=mock_model),
        patch("src.worker.engine.chatbot.explain_agent.Agent", return_value=mock_agent),
    ):
        from src.worker.engine.chatbot.explain_agent import _make_agent
