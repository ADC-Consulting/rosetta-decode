...
def test_make_agent_tensorzero_provider_path() -> None:
    """_make_agent() takes the TensorZero branch when tensorzero_gateway_url is set."""
    from unittest.mock import MagicMock, patch
    ...
    with (
        patch("src.worker.engine.agents.data_step.worker_settings", mock_settings),
        patch("src.worker.engine.agents.data_step.OpenAIProvider", return_value=mock_tz_provider),
        patch(
            "src.worker.engine.agents.data_step.OpenAIChatModel", return_value=mock_model
        ) as mock_oai_model,
        patch("src.worker.engine.agents.data_step.Agent", return_value=mock_agent),
    ):
        from src.worker.engine.agents.data_step import _make_agent
    ...
    assert "tensorzero::" in call_kwargs.kwargs.get("model_name", first_arg)
    assert result is mock_agent


# ── NEW COVERAGE: Azure branch (lines 214-224) ────────────────────────────────


def test_make_agent_azure_branch() -> None:
    """Lines 214-224: _make_agent uses AzureProvider when azure_openai_endpoint is set."""
    from unittest.mock import MagicMock, patch

    with (
        patch(
            "src.worker.engine.agents.data_step.worker_settings",
        ...
        patch("src.worker.engine.agents.data_step.AzureProvider") as mock_azure,
        patch("src.worker.engine.agents.data_step.OpenAIChatModel") as mock_model,
        patch("src.worker.engine.agents.data_step.Agent") as mock_agent_cls,
    ):
