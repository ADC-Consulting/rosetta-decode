...
1. **Update the active feature plan (if any)**Check `docs/plans/latest/` for a file with `Status: in-progress`. If found:
...
3. **Update `journal/DECISIONS.md`** if any architectural decision was made this session.If the decision is non-trivial (new service, data model change, external dependency), also create an ADR in `docs/adr/`.
